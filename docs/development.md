...
| 게임 오버 | `game_over` | 한쪽만 남아 승패가 갈렸거나, 아무도 남지 않아 게임이 끝난 상태 |
| 승자 | `winner` (예약) | 게임 오버 시 이긴 플레이어 |
| 무승부 | `draw` | 승자가 없는 게임 오버 |
| 배틀 | `Battle` | 한 판의 대국. 맵을 소유하고 승패를 판정한다 |
| 모드 | `BattleMode` (예약) | 배틀을 시작할 때 각 자리를 누가 조종할지 짜는 방식 |
| 협공배틀 | `MONSTER` (예약) | 모드. 1P는 사람이, 2P는 NPC로 컴퓨터가 조종한다 |
| 로컬 멀티플레이어 | `LOCAL_MULTI` (예약) | 모드. 1P와 2P를 사람 둘이 조종한다. 한 키보드를 나눠 쓴다 |
| 플레이어 | `Player` (예약) | 배틀에 참가한 한 자리. 캐릭터 하나를 조종한다. 조종자는 사람일 수도 컴퓨터일 수도 있다 |
| 조작키 | `KeyBinding` (예약) | 한 플레이어가 쓰는 키 묶음. 1P는 `R`·`D`·`F`·`G` 이동에 왼쪽 `Shift` 물풍선, 2P는 방향키 이동에 오른쪽 `Shift` 물풍선 |

`(예약)`은 아직 코드에 없는 이름입니다. 그 개념을 처음 만들 때 이 이름을 쓰세요.
