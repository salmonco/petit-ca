...
| 승자 | `winner` | 게임 오버 시 이긴 플레이어 |
| 무승부 | `draw` | 승자가 없는 게임 오버 |
| 배틀 | `Battle` | 한 판의 대국. 맵을 소유하고 승패를 판정한다 |
| 모드 | `BattleMode` | 배틀을 시작할 때 각 자리를 누가 조종할지 짜는 방식 |
| 협공배틀 | `MONSTER` | 모드. 1P는 사람이, 2P는 NPC로 컴퓨터가 조종한다 |
| 로컬 멀티플레이어 | `LOCAL_MULTI` | 모드. 1P와 2P를 사람 둘이 조종한다. 한 키보드를 나눠 쓴다 |
| 온라인 멀티플레이어 | `ONLINE_MULTI` | 모드. 네트워크를 통해 서로 다른 플레이어가 플레이 |
| 조작키 | `GameKey` | 플레이어가 쓰는 키 묶음 |
