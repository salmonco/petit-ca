...
| 걸치기 | — | 캐릭터가 칸 경계에 걸쳐 서서 물줄기를 피하는 것 |
| 갇힘 판정 상자 | `trap_box` | 캐릭터에서 물줄기와 겹치는지 보는 사각형. 스프라이트가 아니라 발 위쪽 하반신만 |
| 앵커 | `continuous_position` | 캐릭터의 논리 위치. 칸의 중앙이고, 발은 여기서 `FEET_FROM_ANCHOR`만큼 아래 |
| 노출된 면 | `TRAP_REACH_EXPOSED` | 물줄기가 이어지지 않은 칸의 면. 칸 경계보다 안쪽에서 끊긴다 |
