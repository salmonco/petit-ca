...
func _character_at(start_position: Vector2i) -> Character:
	return Character.new(start_position, _map)

func test_캐릭터가_위쪽_방향으로_움직인다() -> void:
	var character := _character_at(Vector2i.ONE)
...
func test_캐릭터가_물풍선이_있는_칸에_또_물풍선을_놓으면_무시한다() -> void:
	var character := _character_at(Vector2i(1, 0))
	character.place_water_balloon()
	character.place_water_balloon()
