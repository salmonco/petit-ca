...
func test_키보드_위쪽_방향키를_누르면_캐릭터가_위로_한_칸_이동한다() -> void:
	var initial_position := _main.map.character_positions()[0]
	_main.handle_key(KEY_UP)
	var expected_position := Map.to_pixel(initial_position) + Vector2.UP * Map.PIXELS_PER_CELL
	assert_vector(_main.character_views.get_child(0).position).is_equal(expected_position)
